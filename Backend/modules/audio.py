...
def similarity():
    ...
    try:
        ...
        progress = result.split(' ')[0]
        if (float(progress)<85):
            return jsonify(Reference = ref_to_txt(Random_Path),Recorded = rec_to_txt(Rec_Path),Score = result,Message='محاوله جيده،تابع التقدم')    
        else:
            return jsonify(Reference = ref_to_txt(Random_Path),Recorded = rec_to_txt(Rec_Path),Score = result,Message='احسنت ،تابع التقدم')    
